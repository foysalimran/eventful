...
if ( ! function_exists( 'efp_get_icons' ) ) {
  function efp_get_icons() {
    ...
    if ( ! wp_verify_nonce( $nonce, 'efp_icon_nonce' ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid nonce verification.', 'eventful-pro' ) ) );
    }
    ...
    if ( ! empty( $icon_lists ) ) {
      ...
      echo '<div class="efp-error-text">'. esc_html__( 'No data available.', 'eventful-pro' ) .'</div>';
...
if ( ! function_exists( 'efp_export' ) ) {
  function efp_export() {
    ...
    if ( ! wp_verify_nonce( $nonce, 'efp_backup_nonce' ) ) {
      die( esc_html__( 'Error: Invalid nonce verification.', 'eventful-pro' ) );
    }

    if ( empty( $unique ) ) {
      die( esc_html__( 'Error: Invalid key.', 'eventful-pro' ) );
    }
...
if ( ! function_exists( 'efp_import_ajax' ) ) {
  function efp_import_ajax() {
    ...
    if ( empty( $unique ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid key.', 'eventful-pro' ) ) );
    }

    if ( empty( $data ) || ! is_array( $data ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: The response is not a valid JSON response.', 'eventful-pro' ) ) );
    }
...
if ( ! function_exists( 'efp_chosen_ajax' ) ) {
  function efp_chosen_ajax() {
    ...
    if ( empty( $type ) || empty( $term ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid term ID.', 'eventful-pro' ) ) );
    }

    $capability = apply_filters( 'efp_chosen_ajax_capability', 'manage_options' );

    if ( ! current_user_can( $capability ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: You do not have permission to do that.', 'eventful-pro' ) ) );
    }
