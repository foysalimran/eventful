...
if ( ! class_exists( 'EFP_Profile_Options' ) ) {
  class EFP_Profile_Options extends EFP_Abstract{
    ...
    public function render_profile_form_fields( $profileuser ) {
      ...
      $errors     = ( ! empty( $profile_id ) ) ? get_user_meta( $profile_id, '_taf_errors_'. $this->unique, true ) : array();
      $errors     = ( ! empty( $errors ) ) ? $errors : array();
      $class      = ( $this->args['class'] ) ? ''. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_user_meta( $profile_id, '_taf_errors_'. $this->unique );
      }

      echo '<div class="taf taf-profile-options taf-onload'. esc_attr( $class ) .'">';

      wp_nonce_field( 'taf_profile_nonce', 'taf_profile_nonce'. $this->unique );
    ...
    public function save_profile( $user_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'taf_profile_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'taf_profile_nonce' ) ) {
        return $user_id;
      }
      ...
      $data = apply_filters( "taf_{$this->unique}_save", $data, $user_id, $this );

      do_action( "taf_{$this->unique}_save_before", $data, $user_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_user_meta( $user_id, '_taf_errors_'. $this->unique, $errors );
        }

      }

      do_action( "taf_{$this->unique}_saved", $data, $user_id, $this );

      do_action( "taf_{$this->unique}_save_after", $data, $user_id, $this );
