...
if ( ! class_exists( 'EFP_Nav_Menu_Options' ) ) {
  class EFP_Nav_Menu_Options extends EFP_Abstract{
    ...
    public function wp_nav_menu_item_custom_fields( $menu_item_id, $item, $depth, $args ) {
      ...
      $errors = ( ! empty( $menu_item_id ) ) ? get_post_meta( $menu_item_id, '_taf_errors_'. $this->unique, true ) : array();
      $errors = ( ! empty( $errors ) ) ? $errors : array();
      $class  = ( $this->args['class'] ) ? ' '. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_post_meta( $menu_item_id, '_taf_errors_'. $this->unique );
      }

      echo '<div class="taf taf-nav-menu-options'. esc_attr( $class ) .'">';

        foreach ( $this->sections as $section ) {

          $section_icon  = ( ! empty( $section['icon'] ) ) ? '<i class="taf-nav-menu-icon '. esc_attr( $section['icon'] ) .'"></i>' : '';
    ...
    public function wp_update_nav_menu_item( $menu_id, $menu_item_db_id, $menu_item_args ) {
      ...
      $data = apply_filters( "taf_{$this->unique}_save", $data, $menu_item_db_id, $this );

      do_action( "taf_{$this->unique}_save_before", $data, $menu_item_db_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_post_meta( $menu_item_db_id, '_taf_errors_'. $this->unique, $errors );
        }

      }

      do_action( "taf_{$this->unique}_saved", $data, $menu_item_db_id, $this );

      do_action( "taf_{$this->unique}_save_after", $data, $menu_item_db_id, $this );
