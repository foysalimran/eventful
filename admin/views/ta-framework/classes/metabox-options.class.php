...
if ( ! class_exists( 'EFP_Metabox' ) ) {
  class EFP_Metabox extends EFP_Abstract{
    ...
    public function add_metabox_classes( $classes ) {
      ...
      if ( ! empty( $this->post_formats ) ) {
        ...
        foreach ( $this->post_formats as $format ) {
          $classes[] = 'taf-post-format-'. $format;
        }

        if ( ! in_array( $saved_post_format, $this->post_formats ) ) {
          $classes[] = 'taf-metabox-hide';
        } else {
          $classes[] = 'taf-metabox-show';
        }
      ...
      if ( ! empty( $this->page_templates ) ) {
        ...
        $classes[] = 'taf-page-templates';

        foreach ( $this->page_templates as $template ) {
          $classes[] = 'taf-page-'. preg_replace( '/[^a-zA-Z0-9]+/', '-', strtolower( $template ) );
        }
    ...
    public function add_meta_box_content( $post, $callback ) {
      ...
      $errors    = ( is_object ( $post ) ) ? get_post_meta( $post->ID, '_taf_errors_'. $this->unique, true ) : array();
      $errors    = ( ! empty( $errors ) ) ? $errors : array();
      $theme     = ( $this->args['theme'] ) ? ' taf-theme-'. $this->args['theme'] : '';
      $nav_type  = ( $this->args['nav'] === 'inline' ) ? 'inline' : 'normal';

      if ( is_object ( $post ) && ! empty( $errors ) ) {
        delete_post_meta( $post->ID, '_taf_errors_'. $this->unique );
      }

      wp_nonce_field( 'taf_metabox_nonce', 'taf_metabox_nonce'. $this->unique );

      echo '<div class="taf taf-metabox'. esc_attr( $theme ) .'">';
    ...
    public function save_meta_box( $post_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'taf_metabox_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'taf_metabox_nonce' ) ) {
        return $post_id;
      }
      ...
      $data = apply_filters( "taf_{$this->unique}_save", $data, $post_id, $this );

      do_action( "taf_{$this->unique}_save_before", $data, $post_id, $this );
      ...
      if ( empty( $data ) || ! empty( $request['_reset'] ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_post_meta( $post_id, '_taf_errors_'. $this->unique, $errors );
        }

      }

      do_action( "taf_{$this->unique}_saved", $data, $post_id, $this );

      do_action( "taf_{$this->unique}_save_after", $data, $post_id, $this );
