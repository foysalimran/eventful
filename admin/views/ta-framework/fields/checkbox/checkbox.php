    ...
    public function render() {
      ...
      if ( isset( $this->field['options'] ) ) {
        ...
        if ( is_array( $options ) && ! empty( $options ) ) {
          ...
          echo ( ! empty( $this->field['empty_message'] ) ) ? esc_attr( $this->field['empty_message'] ) : esc_html__( 'No data available.', 'eventful-pro' );
        ...
        echo '<input type="hidden" name="'. esc_attr( $this->field_name() ) .'" value="'. $this->value .'" class="efp--input"'. $this->field_attributes() .'/>';
        echo '<input type="checkbox" name="_pseudo" class="efp--checkbox"'. esc_attr( checked( $this->value, 1, false ) ) . $this->field_attributes() .'/>';
        echo ( ! empty( $this->field['label'] ) ) ? '<span class="efp--text">'. esc_attr( $this->field['label'] ) .'</span>' : '';
        echo '</label>';
        echo '<span class="hide-taxonomy-name" style="display:none;">Hide Taxonomy Name</span>';
      }
