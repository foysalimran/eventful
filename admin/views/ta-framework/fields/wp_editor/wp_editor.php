 ...
 *
 */
if ( ! class_exists( 'EFP_Field_wp_editor' ) ) {
  class EFP_Field_wp_editor extends EFP_Fields {
    ...
    public function render() {
      ...
      echo ( efp_wp_editor_api() ) ? '<div class="efp-wp-editor" data-editor-settings="'. esc_attr( json_encode( $editor_settings ) ) .'">' : '';
    ...
    public function setup_wp_editor_media_buttons() {
      ...
      echo 'var efp_media_buttons = '. json_encode( $media_buttons ) .';';
      echo '</script>';
    ...
    public function setup_wp_editor_settings() {
      ...
      if ( efp_wp_editor_api() && class_exists( '_WP_Editors') ) {
        ...
        $defaults = apply_filters( 'efp_wp_editor', array(
          'tinymce' => array(
            'wp_skip_init' => true
          ),
        ) );
