	...
	class EFP_Field_typography extends EFP_Fields {
		...
		public function enqueue() {
			...
			if ( ! wp_script_is( 'efp-webfontloader' ) ) {
				...
				wp_enqueue_script( 'webfontloader', EFP_URL . 'admin/views/ta-framework/assets/js/webfontloader.min.js', array( 'ta-framework' ), '1.6.28', true );
			...
			if ( $is_google ) {
				...
				if ( $font_weight || $font_style ) {
					...
					if ( ! empty( $style ) ) {
						...
						EFP::$webfonts[ $method ][ $font_family ][ $style ] = $style;
					}
				} else {
					EFP::$webfonts[ $method ][ $font_family ] = array();
				}
				...
				if ( ! empty( $this->value['extra-styles'] ) ) {
					foreach ( $this->value['extra-styles'] as $extra_style ) {
						if ( ! empty( $extra_style ) ) {
								...
								EFP::$webfonts[ $method ][ $font_family ][ $extra_style ] = $extra_style;
						}
					}
				}
