  ...
  class EFP_Field_map extends EFP_Fields {
    ...
    public function render() {
      ...
      $args              = wp_parse_args( $this->field, array(
        'placeholder'    => esc_html__( 'Search...', 'eventful-pro' ),
        'latitude_text'  => esc_html__( 'Latitude', 'eventful-pro' ),
        'longitude_text' => esc_html__( 'Longitude', 'eventful-pro' ),
        'address_field'  => '',
        'height'         => '',
      ) );
      ...
      echo '<div class="efp--map-osm-wrap"><div class="efp--map-osm" data-map="'. esc_attr( json_encode( $settings ) ) .'"'. $style_attr .'></div></div>';
    ...
    public function enqueue() {
      ...
      if ( ! wp_script_is( 'efp-leaflet' ) ) {
        wp_enqueue_script( 'efp-leaflet', esc_url( $this->cdn_url . $this->version .'/dist/leaflet.js' ), array( 'eventful-pro' ), $this->version, true );
      }

      if ( ! wp_style_is( 'efp-leaflet' ) ) {
        wp_enqueue_style( 'efp-leaflet', esc_url( $this->cdn_url . $this->version .'/dist/leaflet.css' ), array(), $this->version );
      }
