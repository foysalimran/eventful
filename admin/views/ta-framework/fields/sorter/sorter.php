...
if ( ! class_exists( 'EFP_Field_sorter' ) ) {
	class EFP_Field_sorter extends EFP_Fields {
		...
		public function render() {
			...
			echo '<div class="taf-sorter" data-depend-id="' . esc_attr( $this->field['id'] ) . '"></div>';

			echo ( $args['disabled'] ) ? '<div class="taf-modules">' : '';

			echo ( ! empty( $args['enabled_title'] ) ) ? '<div class="taf-sorter-title">' . esc_attr( $args['enabled_title'] ) . '</div>' : '';
			echo '<ul class="taf-enabled">';
			...
			if ( $args['disabled'] ) {
				...
				echo '<div class="taf-modules">';
				echo ( ! empty( $args['disabled_title'] ) ) ? '<div class="taf-sorter-title">' . esc_attr( $args['disabled_title'] ) . '</div>' : '';
				echo '<ul class="taf-disabled">';
