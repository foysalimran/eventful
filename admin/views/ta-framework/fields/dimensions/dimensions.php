  ...
  class EFP_Field_dimensions extends EFP_Fields {
    ...
    public function render() {
      ...
      $args = wp_parse_args( $this->field, array(
        ...
        'width_placeholder'  => esc_html__( 'width', 'eventful-pro' ),
        'height_placeholder' => esc_html__( 'height', 'eventful-pro' ),
        'width'              => true,
        'height'             => true,
        'unit'               => true,
        'show_units'         => true,
      ...
      if ( ! empty( $args['width'] ) ) {
        ...
        echo ( ! empty( $args['width_icon'] ) ) ? '<span class="efp--label efp--icon">'. $args['width_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[width]' ) ) .'" value="'. esc_attr( $value['width'] ) .'"'. $placeholder .' class="efp-input-number'. esc_attr( $is_unit ) .'" step="any" />';
      ...
      if ( ! empty( $args['height'] ) ) {
        ...
        echo '<div class="efp--input">';
        echo ( ! empty( $args['height_icon'] ) ) ? '<span class="efp--label efp--icon">'. $args['height_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[height]' ) ) .'" value="'. esc_attr( $value['height'] ) .'"'. $placeholder .' class="efp-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( ! empty( $unit ) ) ? '<span class="efp--label efp--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
      }
