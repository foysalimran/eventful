...
if ( ! class_exists( 'EFP_Field_link_color' ) ) {
  class EFP_Field_link_color extends EFP_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'eventful-pro' ),
        'hover'    => esc_html__( 'Hover', 'eventful-pro' ),
        'active'   => esc_html__( 'Active', 'eventful-pro' ),
        'visited'  => esc_html__( 'Visited', 'eventful-pro' ),
        'focus'    => esc_html__( 'Focus', 'eventful-pro' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="efp-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
