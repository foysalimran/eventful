...
if ( ! class_exists( 'EFUL_Field_icon' ) ) {
	class EFUL_Field_icon extends EFUL_Fields {
		...
		public function render() {
			...
			echo '<span class="eventful-icon-preview' . esc_attr( $hidden ) . '"><i class="' . esc_attr( $this->value ) . '"></i></span>';
			echo '<a href="#" class="button button-primary eventful-icon-add" data-nonce="' . esc_attr( $nonce ) . '">' . esc_html( $args['button_title'] ) . '</a>';
			echo '<a href="#" class="button eventful-warning-primary eventful-icon-remove' . esc_attr( $hidden ) . '">' . esc_html( $args['remove_title'] ) . '</a>';
			echo '<input type="hidden" name="' . esc_attr( $this->field_name() ) . '" value="' . esc_attr( $this->value ) . '" class="eventful-icon-value"' . wp_kses_post( $this->field_attributes() ) . ' />';
			echo '</div>';
		...
		public static function add_footer_modal_icon() {
			?>
		<div id="eventful-modal-icon" class="eventful-modal eventful-modal-icon hidden">
