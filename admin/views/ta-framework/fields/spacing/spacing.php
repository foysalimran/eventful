  ...
  class EFP_Field_spacing extends EFP_Fields {
    ...
    public function render() {
      ...
      if ( ! empty( $args['all'] ) ) {
        ...
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[all]' ) ) .'" value="'. esc_attr( $value['all'] ) .'"'. $placeholder .' class="efp-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        ...
        foreach ( $properties as $property ) {
          ...
          echo '<input type="number" name="'. esc_attr( $this->field_name( '['. $property .']' ) ) .'" value="'. esc_attr( $value[$property] ) .'"'. $placeholder .' class="efp-input-number'. esc_attr( $is_unit ) .'" step="any" />';
          echo ( $unit ) ? '<span class="efp--label efp--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
          echo '</div>';
