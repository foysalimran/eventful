...
if ( is_array( $advanced_filter ) && ! $eventful_query->is_main_query() ) {
	ob_start();
		EFP_Live_Filter::efp_live_filter( $view_options, $query_args, $eventful_gl_id );
		EFP_Live_Filter::efp_author_filter( $view_options, $query_args );
		EFP_Live_Filter::efp_custom_filter_filter( $view_options, $query_args, $eventful_gl_id );
	$filter_bar = ob_get_clean();

	ob_start();
		EFP_Live_Filter::efp_orderby_filter_bar( $view_options, $eventful_query, $eventful_gl_id );
		EFP_Live_Filter::efp_order_filter_bar( $view_options, $eventful_gl_id );
		EFP_Live_Filter::efp_live_search_bar( $view_options, $eventful_gl_id );
