...
if ('carousel_layout' !== $layout && $lazy_load && !is_admin()) {
	...
	$image = sprintf('<img data-efp_src="%1$s" %5$s class="efp-lazyload" width="%2$s"  height="%3$s" alt="%4$s">', $thumb_url, $eventful_image_attr['width'], $eventful_image_attr['height'], $alter_text, $retina_img_attr);
...
<div class="eventful__item--thumbnail">
	<?php if ('none' === $eventful_page_link_type) { ?>
		<a class="ta-efp-thumb" aria-label="<?php echo esc_attr($eventful_image_attr['aria_label']); ?>" <?php echo esc_attr($eventful_link_rel_text); ?>>
		<?php } else { ?>
			<a class="ta-efp-thumb" aria-label="<?php echo esc_attr($eventful_image_attr['aria_label']); ?>" href="<?php the_permalink($post); ?>" target="<?php echo esc_attr($eventful_link_target); ?>" <?php echo esc_attr($eventful_link_rel_text); ?>>
			<?php
		}
		if (empty($eventful_image_attr['video']) && empty($eventful_image_attr['audio'])) {
			...
			?>
				<div class='ta-efp-post-video-thumb-area'><?php echo $eventful_image_attr['video']; ?></div>
			<?php
		} elseif ($eventful_image_attr['audio']) {
			?>
				<div class='ta-efp-post-audio-thumb-area'><?php echo $eventful_image_attr['audio']; ?></div>
			<?php } ?>
			</a>
			<?php
